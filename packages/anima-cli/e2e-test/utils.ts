...
const getCommand = (isUsingBasePath: boolean, tokenPath: string) =>
  isUsingBasePath
    ? `node ./dist/src/cli.js sync -s "s3://anima-uploads/e2e-tests/storybook-sample-base-path" -t ${token} --debug -b /styleguide --design-tokens ${tokenPath}`
    : `node ./dist/src/cli.js sync -s "s3://anima-uploads/e2e-tests/storybook-sample" -t ${token} --debug --design-tokens ${tokenPath}`;
...
const syncOnlyTokens = ({
  ...
  const output = execSync(
    `node ./dist/src/cli.js sync -t ${token} --debug --design-tokens ${DESIGN_TOKENS_TEST_FILES[designTokenVersion]}`,
  );
  const result = storyHashReg.exec(output.toString());
  if (!result) throw new Error('storybook hash not found');
  const [, storybookHash] = result;
