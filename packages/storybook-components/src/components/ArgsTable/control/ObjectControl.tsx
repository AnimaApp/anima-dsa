...
const ActionIcon = styled(Icons)(
  ({ theme, icon, disabled }: ActionIconProps) => ({
    display: 'inline-block',
    ...
    '&:hover': disabled
      ...
      : {
          color:
            icon === 'subtract' ? theme.color.negative : theme.color.ancillary,
        },
    'svg + &': {
      marginLeft: 0,
    },
...
const getCustomStyleFunction: (theme: Theme) => JsonTreeProps['getStyle'] =
  (theme) => () => ({
    name: {
      ...
      {!showRaw ? (
        <JsonTree
          ...
          inputElement={(_: any, __: any, ___: any, key: string) =>
            key ? (
              <Input onFocus={selectValue} onBlur={dispatchEnterKey} />
            ) : (
              <Input />
            )
          }
